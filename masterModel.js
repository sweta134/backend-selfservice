...
module.exports = {
  ...
  get_dashboard_details: function (connection, id, controllerCallback) {
    ...
    var sql =
      "SELECT SUM(C.amount) AS CURRENT_DUE FROM student_master A INNER JOIN Invoice B on A.studentID = B.studentid INNER JOIN Invoice_Componets C on B.invoice_no = C.invoice_id INNER JOIN fees_components_master D on C.fees_components_master_id = D.id INNER JOIN academic_session E on B.academic_session_id = E.id WHERE B.studentid='"+studentid+"' AND B.`status`='1'"
    connection.query(sql, (err, result) => {
      sql =
        "SELECT SUM(C.amount) AS PREVIOUS_DUE FROM student_master A INNER JOIN Invoice B on A.studentID = B.studentid INNER JOIN Invoice_Componets C on B.invoice_no = C.invoice_id INNER JOIN fees_components_master D on C.fees_components_master_id = D.id INNER JOIN academic_session E on B.academic_session_id = E.id WHERE B.studentid='"+studentid+"' AND B.`status`='1'AND B.academic_session_id = (SELECT id FROM academic_session WHERE is_current_session='0' LIMIT 1) LIMIT 5;"
      connection.query(sql, (err, result1) => {
        sql = "SELECT DISTINCT(payable_amount) AS TOTAL_COMMITMENT from transaction_master WHERE student_id= '"+studentid+"' and is_synced= 'Y'"

        connection.query(sql, (err, result2) => {
          sql = "SELECT SUM(paid_amount) AS TOTAL_PAID_AMOUNT FROM transaction_master WHERE student_id='"+studentid+"' AND is_synced='Y' AND chec='Paid'"
          connection.query(sql, (err, result3) => {
            sql = "SELECT SUM(C.amount) AS UPCOMING_DUE FROM student_master A INNER JOIN Invoice B on A.studentID = B.studentid INNER JOIN Invoice_Componets C on B.invoice_no = C.invoice_id INNER JOIN fees_components_master D on C.fees_components_master_id = D.id INNER JOIN academic_session E on B.academic_session_id = E.id WHERE B.studentid='"+studentid+"' AND B.`status`='1'AND B.academic_session_id = (SELECT id FROM academic_session WHERE is_current_session='0' LIMIT 1 OFFSET 1 ) LIMIT 5;"
